...
if (isset($_POST['btnreg'])) {
    ...
    $fechaUsuario = new DateTime($fechaNacimiento);
    ...
    if (validar_contraseña($nuevaContra)) {
        ...
        try {
            ...
            // Capturar el error del trigger (por ejemplo, si el usuario ya existe)
            echo "<script>alert('Error: " . addslashes($e->getMessage()) . "'); window.history.back();</script>";
        }

        $stmt->close();
    } else {
        echo "<script>alert('La contraseña debe tener al menos 5 caracteres, una mayúscula y un carácter especial.'); window.history.back();</script>";
        exit();
    }
}
?>
